...
function XMPTarget(appID) {
    'use strict';
    var xmpFile, docRef, thumb, xmpMetadata, clipType, itemIsSequence, isForceOpened = false;
    this._appID = appID;
            ...
            if (appID === "PPRO") {
                ...
                if (docRefs.length > 0) {
                    for (var i = 0; i < docRefs.length; i++) {
                        var clipItem = clipType === undefined && !itemIsSequence ? docRefs[i].getMediaPath() : docRefs[i].nodeId;
        ...
        switch (this._appID) {
            ...
            case ("PPRO"):
                ...
                if (docRef) {
                    ...
                    xmpMetadata = clipType === undefined ? docRef.getXMPMetadata():docRef.getProjectMetadata()
                    return new XMPMeta(xmpMetadata)
                }
                break;
                ...
                case ("KBRG"):
                    if (thumb !== undefined) {
                        if (thumb.core.itemContent.canSetXmp === true) {
                            ...
                            var isDynamicMediaType = thumb.core.itemContent.dynamicMediaType !== 1
...
function XMPAdapter(xmpTarget) {
    ...
    var getStructObj = function(namespace, property) {
        var obj = {},
            iter = xmpMeta.iterator(XMPConst.ITERATOR_JUST_CHILDREN, namespace, property),
            item = iter.next();
        while (item) {
    ...
    //public
    XMPAdapter.prototype.getLocalizedValue = function(namespace, property, locale) {
        if (xmpMeta) {
            if (this.doesNamespaceExist(namespace)) {
                try {
                    //Get property value
                    ...
                    if (prop !== undefined) {
                        if (prop.options & XMPConst.PROP_IS_ARRAY) {
                            ...
                            if (arrCnt > 0) {
                                var result = xmpMeta.getLocalizedText(namespace, property, "", locale);
    ...
    //public
    XMPAdapter.prototype.set = function(namespace, prefix, property, value, arrayType, shouldOverwrite) {
        ...
        if (xmpMeta) {
            ...
            try {
                ...
                clipType !== undefined && app.project.addPropertyToProjectMetadataSchema(property, property, clipType)
                ...
                switch (Object.prototype.toString.call(value)) {
                    ...
                    case '[object Array]':
                        ...
                        for (var i = 0; i < value.length; i++) {
                            if (Object.prototype.toString.call(value[i]) === '[object Object]') {
                                ...
                                var arrayPath = XMPUtils.composeArrayItemPath(namespace, property, i + 1);
                                if(value[i].isDeleted === true && shouldOverwrite === false){
                                    delIndex.push(i + 1)
                                }else{
                                    this.set(namespace, prefix, arrayPath, value[i].entries, arrayType, shouldOverwrite);
                                }
                            } else {
                                if(xmpMeta.countArrayItems(namespace, property) > 0 && !isEmpArr){
                    ...
                    case '[object Object]':
                        ...
                        for (var structProp in value) {
                            ...
                            XMPMeta.registerNamespace(value[structProp].namespace, value[structProp].prefix);
                            // using propertName shortcut to write Struct object
                            var strutFieldPath = XMPUtils.composeStructFieldPath(namespace, property, value[structProp].namespace, structProp)
                            this.set(namespace, prefix, strutFieldPath, value[structProp].value, value[structProp].arrayType, value[structProp].shouldOverwrite || shouldOverwrite)
                        }
                        break;
                    default:
    ...
    XMPAdapter.prototype.setLocalizedValue = function(namespace, prefix, property, locale, isDefaultLocale, value) {
        if (xmpMeta) {
            if (!this.doesNamespaceExist(namespace)) {
                XMPMeta.registerNamespace(namespace, prefix);
            }
            try {
                ...
                if (!(prop && prop.options & XMPConst.ARRAY_IS_ALTERNATIVE)) {
                    xmpMeta.deleteProperty(namespace, property);
                    xmpMeta.setProperty(namespace, property, null, XMPConst.ARRAY_IS_ALTERNATIVE);
                    isNew = true
                }

                if (isDefaultLocale === false) {
                    if (isNew === false) {
                        defaultValue = xmpMeta.getArrayItem(namespace, property, 1).toString()
                    }
                }
                ...
                xmpMeta.setQualifier(namespace, arr1IndexName, XMPConst.NS_XML, 'lang', 'x-default');
            } catch (err) {
                throw err.message
            }
        }
    ...
    //public
    XMPAdapter.prototype.deleteLocalizedValue = function(namespace, prefix, property, locale, isDefaultLocale) {
        if (xmpMeta) {
            try {
                XMPMeta.registerNamespace(namespace, prefix);
                if (xmpMeta.doesPropertyExist(namespace, property)) {
                    ...
                    for (var i = 1; i <= items.length; i++) {
                        ...
                        var qNameValue = xmpMeta.getQualifier(namespace, qNameIndex, XMPConst.NS_XML, 'lang');
    ...
    XMPAdapter.prototype.commit = function(filename) {
        if (xmpMeta) {
            ...
            try {
                var updatedPacket = xmpMeta.serialize(XMPConst.SERIALIZE_OMIT_PACKET_WRAPPER | XMPConst.SERIALIZE_USE_COMPACT_FORMAT);
...
function XMPCEPHelper(appID) {
    ...
    XMPCEPHelper.prototype.getXMP = function(args) {
        ...
        try {
            ...
            return JSON.stringify({
                status: 'error',
                message: "Get - " + filename + ":" + namespace + ":" + property + " - " + err,
                result: null
            });
        }
    }
    ...
    XMPCEPHelper.prototype.setXMP = function(args) {
        ...
        try {
            ...
            if (altLang && altLang.length > 0) {
                xmp.setLocalizedValue(namespace, prefix, property, altLang, altLangDefault, value);
            } else {
            ...
            return JSON.stringify({
                status: 'error',
                message: "Set - " + filename + ":" + namespace + ":" + property + " - " + err
            })
        }
    }
    ...
    XMPCEPHelper.prototype.deleteXMP = function(args) {
        ...
        try {
            ...
            if (altLang && altLang.length > 0) {
                xmp.deleteLocalizedValue(namespace, prefix, property, altLang, altLangDefault)
            } else {
            ...
            return JSON.stringify({
                status: "error",
                message: "Delete - " + filename + ":" + namespace + ":" + property + " - " + err
            })
        }
    }
}
